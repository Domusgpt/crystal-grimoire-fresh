...
async function deleteCollectionRecursive(collectionRef, batchSize = 500) {
...
exports.createUserDocument = onDocumentCreated('users/{userId}', async (event) => {
  try {
    ...
    const profile = {
      displayName: userData.profile?.displayName || userData.displayName || 'Crystal Seeker',
    ...
    const defaultPlan = resolvePlanDetails('explorer');
    await userRef.collection('plan').doc('active').set({
      ...defaultPlan,
      provider: 'system',
      priceId: null,
      createdAt: FieldValue.serverTimestamp(),
    ...
    await userRef.collection('economy').doc('balance').set({
      credits: 0,
      earnedLifetime: 0,
      createdAt: FieldValue.serverTimestamp(),
      updatedAt: FieldValue.serverTimestamp(),
...
exports.deleteUserAccount = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      for (const name of targetedCollections) {
        await deleteCollectionRecursive(userRef.collection(name));
      }

      const economySnapshot = await userRef.collection('economy').get();
      for (const doc of economySnapshot.docs) {
        await deleteCollectionRecursive(doc.ref.collection('transactions'));
        await doc.ref.delete();
      }

      await deleteQueryBatch(db.collection('usage').where('userId', '==', userId));
      await deleteQueryBatch(db.collection('usage_logs').where('userId', '==', userId));
      await deleteQueryBatch(db.collection('guidance_sessions').where('userId', '==', userId));
      await deleteQueryBatch(db.collection('crystal_identifications').where('userId', '==', userId));
  ...
  async (request) => {
    ...
    try {
      ...
      if (action === 'crystal_identification') {
        updates['profile.stats.totalIdentifications'] = FieldValue.increment(1);
        updates['profile.stats.monthlyIdentifications'] = FieldValue.increment(1);
      } else if (action === 'metaphysical_query') {
        updates['profile.stats.metaphysicalQueries'] = FieldValue.increment(1);
      }

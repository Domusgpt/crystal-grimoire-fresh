...
- `SupportService` mirrors the callable transitions client-side so QA can create tickets, add comments, and retry updates offline; once `ENABLE_SUPPORT_TICKETS=true` and Firebase is configured the same API syncs with Cloud Functions automatically.【F:lib/services/support_service.dart†L1-L588】【F:test/services/support_service_test.dart†L1-L84】

...
- **Motion principles**:
  ...
  3. **Holographic reveals**: Cards fade/scale in with prism glows; holographic canvas visualizer initializes on entry, tears down on exit.
  4. **Breathing rhythm**: Easing via `power3.inOut` and `sine.inOut`; micro hover transforms.
  5. **Resource aware**: Visualizer pauses for reduced-motion or when scrolled away.

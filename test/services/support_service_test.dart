...
void main() {
  group('SupportService offline fallback', () {
    ...
    test('synchronizePending short-circuits when Firebase unavailable', () async {
      final ticket = await service.createTicket(
        userId: 'user-4',
        subject: 'Offline sync',
        description: 'Waiting for connectivity.',
      );

      final results = await service.synchronizePending('user-4');
      expect(results, hasLength(1));
      expect(results.first.id, equals(ticket.id));
      expect(
        service.lastError,
        contains('Support tickets sync unavailable'),
      );
    });

    test('pendingCommentsForSync skips seed comment for new tickets', () async {
      final ticket = await service.createTicket(
        userId: 'user-5',
        subject: 'Comment sync',
        description: 'Initial details go here.',
      );

      await service.addComment(
        ticket.id,
        userId: 'user-5',
        message: 'Additional offline update.',
      );

      final hydrated = service.getTicket(ticket.id)!;
      final comments = service.pendingCommentsForSync(
        hydrated,
        newTicketCreated: true,
      );

      expect(comments, hasLength(1));
      expect(comments.single.message, contains('Additional offline update.'));
    });
  });
}
